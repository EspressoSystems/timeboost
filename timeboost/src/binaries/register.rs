...
use reqwest::Client;
use timeboost_config::{CommitteeConfig, HTTP_API_PORT_OFFSET};
    ...
    match args.action {
            ...
            let members = config
                ...
                .map(|m| {
                    ...
                    Ok::<_, anyhow::Error>(CommitteeMemberSol {
                        ...
                        batchPosterAddress: todo!(), // m.batch_poster_api.to_string(),
                        sigKeyAddress: todo!(),      // public_key_to_address(pub_key),
                    })
                })
                .collect::<Result<Vec<_>, _>>()?;

            let _tx_receipt = contract
                .setNextCommittee(config.effective.into(), members)
                .send()
                .await?
                .get_receipt()
                .await?;
        ...
        Action::ThresholdEncKey => {
            ...
            let urls = config
                ...
                .iter()
                .map(|m| {
                    let addr = m.address.with_offset(HTTP_API_PORT_OFFSET).clone();
