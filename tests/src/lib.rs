...
use sailfish::{consensus::{committee::StaticCommittee, Consensus}, logging, sailfish::{generate_key_pair, Sailfish}, types::{NodeId, PublicKey}};
...
pub fn make_consensus_nodes(num_nodes: u64) -> Vec<(NodeId, Consensus)> {
    let keys = (0 .. num_nodes).map(|i| generate_key_pair(SEED, i)).collect::<Vec<_>>();
    let committee = StaticCommittee::new(keys.iter().map(|(_, k)| k).cloned().collect());
    keys.into_iter().enumerate().map(|(i, (sk, pk))| (NodeId::from(i as u64), Consensus::new(pk, sk, committee.clone()))).collect()
}
...
impl Group {
    pub fn new(size: u16) -> Self {
        ...
        for i in 0..size {
            ...
            let sailfish = Sailfish::new(i as u64, pk, sk, multiaddr!(Ip4([0,0,0,0]), Tcp(8000 + i))).unwrap();
            nodes.push(sailfish);
            vcgfs.push(cfg)
        }
        ...
        let staked_nodes: Vec<PeerConfig<PublicKey>> = vcgfs
            .iter()
            .map(|c| c.public_config())
            .collect();
