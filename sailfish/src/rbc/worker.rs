...
use super::{Command, RbcConfig, Digest, Protocol};

type Result<T> = std::result::Result<T, RbcError>;
type Sender<B> = mpsc::Sender<Message<B, Validated>>;
type Receiver<B> = mpsc::Receiver<Command<B>>;

/// A worker is run by `Rbc` to perform the actual work of sending and
/// delivering messages.
pub struct Worker<C, B> {
    /// RBC configuration.
    config: RbcConfig,
    ...
    buffer: BTreeMap<RoundNumber, Messages<B>>,
...
struct Tracker<B> {
    ...
    message: Item<Message<B, Validated>>,
...
impl<C: RawComm, B> Worker<C, B> {
    pub fn new(tx: Sender<B>, rx: Receiver<B>, cfg: RbcConfig, nt: C) -> Self {
        Self {
...
impl<C: RawComm, B: Clone + Committable + Serialize + DeserializeOwned> Worker<C, B> {
    ...
    async fn on_broadcast(&mut self, msg: Message<B, Validated>) -> Result<()> {
    ...
    /// 1:1 communication.
    #[instrument(level = "trace", skip_all, fields(n = %self.label, m = %msg))]
    async fn on_send(&mut self, to: PublicKey, msg: Message<B, Validated>) -> Result<()> {
        let proto = Protocol::Send(Cow::Borrowed(&msg));
        let bytes = serialize(&proto)?;
        let digest = Digest::new(&msg);
    ...
    /// Start RBC broadcast.
    #[instrument(level = "trace", skip_all, fields(n = %self.label, m = %msg))]
    async fn on_outbound(&mut self, msg: Message<B, Validated>) -> Result<()> {
    ...
    async fn on_message(
        ...
        let ack = Protocol::<'_, B, Validated>::Ack(env);
    ...
    #[instrument(level = "trace", skip_all, fields(n = %self.label, m = %msg))]
    async fn on_propose(&mut self, msg: Message<B, Unchecked>) -> Result<()> {
        let Some(msg) = msg.validated(&self.config.committee) else {
            return Err(RbcError::InvalidMessage);
        };
        ...
        match tracker.status {
            ...
            Status::Init => {
                ...
                let vote = Protocol::<'_, B, Validated>::Vote(env, false);
    ...
    async fn on_vote(&mut self, env: Envelope<Digest, Unchecked>, done: bool) -> Result<()> {
        ...
        match tracker.status {
            ...
            | Status::SentVote => match tracker.votes.add(env.into_signed()) {
                ...
                Ok(Some(cert)) => {
                    ...
                    if let Some(msg) = &tracker.message.item {
                        ...
                        let m = Protocol::<'_, B, Validated>::Cert(e);
                        let b = serialize(&m)?;
                        ...
                        let m = Protocol::<'_, B, Validated>::Get(e);
                        let b = serialize(&m)?;
            ...
            Status::ReachedQuorum if tracker.message.item.is_none() => {
                let m = Protocol::<'_, B, Validated>::Get(Envelope::signed(digest, &self.config.keypair, false));
                let b = serialize(&m)?;
            ...
            Status::ReachedQuorum | Status::RequestedMsg | Status::Delivered => {
                if done {
                    ...
                    let env = Envelope::signed(digest, &self.config.keypair, false);
                    let vote = Protocol::<'_, B, Validated>::Vote(env, true);
                    let bytes = serialize(&vote)?;
