...
use multisig::{Envelope, PublicKey, Unchecked};
use serde::Serialize;
...
impl Multiplex {
    pub fn go(label: PublicKey, mut net: Overlay) -> (DecryptReceiver, BlockReceiver, Self) {
        // Channel for multiplexed messages to/from the overlay network.
        let (obound_tx, mut obound_rx) = mpsc::channel(100);
        // Channel reserved for the decrypter
        let (dec_tx, dec_rx) = mpsc::channel(100);
