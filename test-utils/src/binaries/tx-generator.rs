...
const KEY_MANAGER: &str = "0x2bbf15bc655c4cc157b769cfcb1ea9924b9e1a35";
const INBOX: &str = "0xa0f3a1a4e2b2bcb7b48c8527c28098f207572ec1";

#[derive(Parser, Debug)]
struct Args {
    #[clap(long, short, default_value = KEY_MANAGER)]
    key_manager_contract: Address,

    #[clap(long, short, default_value_t = 0)]
    committee_id: u64,

    #[clap(long, default_value_t = 31337)]
    parent_chain_id: u64,
    ...
    #[clap(long, short, default_value = INBOX, requires = "rpc_url")]
    parent_ibox_contract: Address,

    #[clap(long, short, default_value = DEV_ACCOUNT, requires = "rpc_url")]
    dev_account: String,

    #[clap(long, default_value_t = 5, requires = "rpc_url")]
    nitro_senders: u32,
}
...
async fn main() -> Result<()> {
    ...
    let c = CommitteeInfo::fetch(
        args.parent_rpc_url.clone(),
        args.key_manager_contract,
        args.committee_id.into(),
    )
    .await
    ...
    let nitro_cfg = args.rpc_url.map(|rpc_url| {
        NitroConfig::builder()
            ...
            .bridge_addr(args.parent_ibox_contract)
            .build()
    });
    ...
    let config = TxGeneratorConfig::builder()
        ...
        .parent_url(args.parent_rpc_url)
        .parent_id(args.parent_chain_id)
