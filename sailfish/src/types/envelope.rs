...
use crate::{consensus::committee::StaticCommittee, types::{PublicKey, SecretKey, Signature}};
...
impl<D: Committable> Envelope<D> {
    ...
    pub fn sign(d: D,  sk: &SecretKey, pk: PublicKey) -> Result<Self> {
        let c = d.commit();
        let s = PublicKey::sign(sk, c.as_ref())?;
        Ok(Self::new(d, s, pk))
    }

    pub fn is_valid(&self, membership: &StaticCommittee) -> bool {
        membership.committee().contains(&self.signing_key)
            && self.signing_key.validate(&self.signature, self.commitment.as_ref())
    }
